        ...
        for j in range(toponode):
            ...
            for l1 in r1:
                ...
                if(s1==srcip and d1==alignip and p1=='ICMP' and count1==0):
                    time1=keys[1]
                    ##注：在eval1.py和feature1.py中有这一点不同
                    count1=1
        ...
        for j in range(toponode):
            ...
            for l0 in r0:
                ...
                if(time_now<=time_end and time_now>=time_start):
                    f.write(keys[1]+'\t'+keys[2]+'\t'+keys[3]+'\t'+keys[4]+'\t'+keys[5]+'\t'+keys[6]+'\t'+header+'\n')
            f.close()
            f0.close()
